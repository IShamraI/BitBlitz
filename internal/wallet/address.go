...
func Bitcoin_GenerateKeypair() (prikey BitcoinPrivateKey, pubkey BitcoinPublicKey, err error) {
	...
	if err != nil {
		return prikey, pubkey, fmt.Errorf("generating random private key")
	}
	...
	if !curve.isOnCurve(Q) {
		return prikey, pubkey, fmt.Errorf("catastrophic math failure")
	}
...
func Write_Prikey(prikey BitcoinPrivateKey, dir string, label string) (err error) {
	...
	if dir == "" || (err == nil && !fi.IsDir()) {
		return fmt.Errorf("private key directory path not a directory")
	} else if err != nil && os.IsNotExist(err) {
		return fmt.Errorf("private key directory path does not exist")
	} else if err != nil {
	...
	for {
		...
		if err != nil && !os.IsNotExist(err) {
			return fmt.Errorf("stat'ing new private key file: %s", err)
